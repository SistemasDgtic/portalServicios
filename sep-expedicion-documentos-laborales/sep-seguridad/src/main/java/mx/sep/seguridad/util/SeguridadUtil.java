...
public class SeguridadUtil {
    ...
    public static UsuarioSeguridad getUsuarioActual() {
        ...
        UsuarioSeguridad us = new UsuarioSeguridad(cud.getUsername(), "", cud.getNombre(), cud.isEnabled(), cud.isAccountNonLocked());
        us.setFechaAcceso( new Date() );
        return us;
    }
    
    public static String getUnicoRol() {
		if (SecurityContextHolder.getContext().
			getAuthentication() == null
			|| !SecurityContextHolder.getContext().
			getAuthentication().
		...
		for (GrantedAuthority grantedAuthority : authorities) {
			nombreRol = grantedAuthority.getAuthority();
		}
		return nombreRol;
	}
}
